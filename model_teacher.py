...
from crossattn_module import Cross_Block
...
class HybridTokenMixer(nn.Module):
    def __init__(self,
        ...
        self.global_unit = Attention_module(dim=dim // 2, num_heads=num_heads)
...
class Block(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = x + self.drop_path(self.token_mixer(self.norm1(x), self.relative_pos_enc))
...
class VisionTransformer(nn.Module):
    def __init__(self,
        ...
        self.patch_embed_B0 = PatchEmbed(img_size=img_size,
                                         patch_size=32,
                                         in_chans=in_chans,
                                         embed_dim=embed_dims[0])
        ...
        self.patch_embed_B1 = PatchEmbed(img_size=img_size // 32,
                                         patch_size=2,
                                         in_chans=embed_dims[0],
                                         embed_dim=embed_dims[1])
        ...
        self.patch_embed_B2 = PatchEmbed(img_size=img_size // 64,
                                         patch_size=2,
                                         in_chans=embed_dims[1],
                                         embed_dim=embed_dims[2])
        ...
        # Stage-3, concat
        self.patch_embed_concat = PatchEmbed(img_size=img_size // 128,
                                             patch_size=2,
                                             in_chans=embed_dims[2],
                                             embed_dim=embed_dims[3])
        self.block_concat = nn.Sequential(*[
        ...
        # cross attention
        self.cross_attn_0 = Cross_Block(dim=embed_dims[0], num_heads=1, fp_size=self.fp_size[0])
        self.cross_attn_1 = Cross_Block(dim=embed_dims[1], num_heads=1, fp_size=self.fp_size[1])
        self.cross_attn_2 = Cross_Block(dim=embed_dims[2], num_heads=1, fp_size=self.fp_size[2])
        ...
        path_dim = 128
        """
        relu but no batchnorm, the student would be nan, but teacher is higher than relu+batchnorm
    ...
    def forward(self, x_a, x_b):
        ...
        x_b0 = self.patch_embed_B0(x_b)
        x_a0 = self.block_A0(x_a0)
        x_b0 = self.block_B0(x_b0)
        cross_a0, cross_b0, self.attn_a0, self.attn_b0 = self.cross_attn_0(x_a0, x_b0)
        f0 = cross_a0.flatten(1)

        # a1:[B, 96, 32, 32]
        x_a1 = self.patch_embed_A1(cross_a0)
        x_b1 = self.patch_embed_B1(cross_b0)
        x_a1 = self.block_A1(x_a1)
        x_b1 = self.block_B1(x_b1)
        cross_a1, cross_b1, self.attn_a1, self.attn_b1 = self.cross_attn_1(x_a1, x_b1)
        f1 = cross_a1.flatten(1)

        # a2:[B, 224, 16, 16]
        x_a2 = self.patch_embed_A2(cross_a1)
        x_b2 = self.patch_embed_B2(cross_b1)
        x_a2 = self.block_A2(x_a2)
        x_b2 = self.block_B2(x_b2)
        cross_a2, cross_b2, self.attn_a2, self.attn_b2 = self.cross_attn_2(x_a2, x_b2)
        f2 = cross_a2.flatten(1)

        # out_concat: [B, 448, 8, 8]
        cross_concat = cross_a2 + cross_b2
        out_concat = self.patch_embed_concat(cross_concat)
        out_concat = self.block_concat(out_concat)

        f3 = self.avg_final(out_concat).flatten(1)
